...
- ⏳ **Phase 5** — Block duplication (RYS-style) — **NOT STARTED**
...
1. **Phase 5 — RYS-style layer duplication** — no training required, purely uses the existing importance maps. Duplicate top-scoring transformer blocks, measure MMLU. Natural next step that doesn't need VRAM headroom.
2. **Budget-neutral prune + duplicate** — prune low-importance tiles, duplicate high-importance blocks so parameter count roughly matches original. The "full RMP" idea.
3. **Sub-10% sparsity on Qwen** — below the accuracy knee. Less speedup but might preserve real signal. Worth measuring for a quality-focused operating point.
...
1. Read `FINDINGS.md` sections 9-13 for the kernel story, 14-15 for the LoRA story, or this doc for a summary
...
4. For RYS duplication (the next natural experiment that fits on 8 GB) → new notebook 6, reusing notebook 4's load-once infrastructure
...
├── FINDINGS.md                     # Chronological findings (15 sections)
├── README.md                       # This file
